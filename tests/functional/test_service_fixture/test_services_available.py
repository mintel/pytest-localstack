...
class TestBotocore:
    ...
    def test_apigateway_available(
        self, localstack: pytest_localstack.session.LocalstackSession
    ):
        client = localstack.botocore.client("apigateway")
        result = client.get_rest_apis()
        assert result["items"] == []

    def test_cloudformation_available(
        self, localstack: pytest_localstack.session.LocalstackSession
    ):
    ...
    def test_dynamodbstreams_available(
        self, localstack: pytest_localstack.session.LocalstackSession
    ):
